    ...
    command:
        ...
        (rawArgs: string | null, player: Player): void => {
            ...
            if (!hasValue(targetRoom)) {
                sayAt(
                    player,
                    "You try really hard to teleport before realizing you have no idea where you're going."
                );
            ...
            player.followers.forEach((follower: Character) => {
                ...
                if (!isNpc(follower)) {
                    sayAt(
                        follower as Player,
                        `You stop following ${player.name}.`
                    );
                }
            });
            ...
            player.moveTo(targetRoom, () => {
                sayAt(
                    player,
                    '{green.bold You snap your finger and instantly appear in a new room.}'
                );
