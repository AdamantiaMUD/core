...
const getCombatantsDisplay = (entity: Character): string => {
    const combatantsList = [...entity.combatants.values()]
...
const lookRoom = (state: GameState, player: Player): void => {
    ...
    room.items.forEach(item => {
        if (item.hasBehavior('resource')) {
            ...
            sayAt(player, `[${ItemUtil.qualityColorize(item, 'Resource')}] <magenta>${item.roomDesc}</magenta>`);
        }
        else {
            /* eslint-disable-next-line max-len */
            sayAt(player, `[${ItemUtil.qualityColorize(item, 'Item')}] <magenta>${item.roomDesc}</magenta>`);
        }
    });
