...
export const cmd: CommandDefinitionFactory = {
    ...
    command:
        ...
        (rawArgs: string | null, player: Player): void => {
            ...
            sayAt(
                player,
                `{bold ${center(60, `${player.name}, level ${player.level} ${pClass.name}`, 'green')}}`
            );
            ...
            sayAt(
                ...
                sprintf(
                    ' %6s:{bold %5s} - {bold %-5s} |',
                    'Damage',
                    `${weaponDamage.min}-${weaponDamage.max}`
                )
            );
            at(player, sprintf('%37s', '|'));
            sayAt(
                ...
                sprintf(
                    ' %6s: {bold %12s} |',
                    'Speed',
                    center(12, `${getWeaponSpeed(player)} sec`)
                )
            );
            ...
            sayAt(
                ...
                sprintf(
                    '%22s| {bold %10s} |',
                    '',
                    player.getMeta<number>('currencies.gold') ?? 0
                )
            );
            printStat('int', false);
            sayAt(player, sprintf('%36s', `'${line(12)}'`));
