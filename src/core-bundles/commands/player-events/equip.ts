...
export const evt: MudEventListenerDefinition<
    [Player, CharacterEquipItemPayload]
> = {
    name: CharacterEquipItemEvent.getName(),
    listener:
        (
            state: GameStateData
        ): MudEventListener<[Player, CharacterEquipItemPayload]> =>
        (player: Player, { slot, item }: CharacterEquipItemPayload): void => {
            ...
            player.addEffect(
                state.effectFactory.create('equip', config, effectState)
            );
        },
};
