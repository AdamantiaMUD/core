...
export const evt: PlayerEventListenerDefinition<UpdateTickPayload> = {
    ...
    listener:
        (state: GameStateData): PlayerEventListener<UpdateTickPayload> =>
        (player: Player): void => {
            if (
                player.commandQueue.hasPending &&
                player.commandQueue.lagRemaining <= 0
            ) {
                sayAt(player);
            ...
            const lastCommandTime: number =
                player.getMeta<number>('lastCommandTime') ?? Infinity;
            const timeSinceLastCommand = Date.now() - lastCommandTime;
            const maxIdleTime =
                Math.abs(state.config.get<number>('maxIdleTime', Infinity)!) *
                60000;
            ...
            if (timeSinceLastCommand > maxIdleTime) {
                player.save(() => {
                    sayAt(
                        player,
                        `You were kicked for being idle for more than ${maxIdleTime / 60000} minutes!`
                    );
                    sayAtExcept(
                        player.room!,
