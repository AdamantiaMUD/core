...
export const evt: PlayerEventListenerDefinition<PlayerMovePayload> = {
    ...
    listener:
        (state: GameStateData): PlayerEventListener<PlayerMovePayload> =>
        (player: Player, { roomExit }: PlayerMovePayload): void => {
            ...
            if (!hasValue(player.room)) {
                ...
                sayAt(
                    player,
                    'Where are you? You have to be somewhere before you can go anywhere!'
                );
            ...
            const door: Door | null =
                oldRoom.getDoor(nextRoom) ?? nextRoom?.getDoor(oldRoom) ?? null;
