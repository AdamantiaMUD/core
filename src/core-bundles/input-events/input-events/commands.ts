...
} from '../../../lib/commands/errors/index.js';
import {
    NoMessageError,
    NoPartyError,
    NoRecipientError,
...
export const evt: StreamEventListenerFactory<CommandLoopPayload> = {
    ...
    listener:
        (state: GameStateData): StreamEventListener<CommandLoopPayload> =>
        (
                ...
                try {
                    ...
                    switch (result.type) {
                        case CommandType.MOVEMENT:
                            player.dispatch(
                                new PlayerMoveEvent(
                                    cast<PlayerMovePayload>(result.payload)
                                )
                            );
                        ...
                        case CommandType.COMMAND: {
                            if (!hasValue(result.command)) {
                                throw new Error(
                                    'Something weird happened. I dunno what'
                                );
                            }

                            const { requiredRole = PlayerRole.PLAYER } =
                                result.command;
