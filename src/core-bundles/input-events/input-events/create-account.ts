...
export const evt: StreamEventListenerFactory<CreateAccountPayload> = {
    ...
    listener:
        (): StreamEventListener<CreateAccountPayload> =>
        (
            ...
            stream.write(
                `{bold Do you want your account's username to be {yellow ${name}}?} {cyan [y/n]} `
            );

            stream.socket.once('data', (buf: Buffer) => {
                const data = buf.toString('utf8').trim().toLowerCase();
                ...
                if (data === 'y' || data === 'yes') {
                    ...
                    stream.dispatch(
                        new ChangePasswordEvent({
                            account: newAccount,
                            nextEvent: new CreateCharacterEvent({
                                account: newAccount,
                            }),
                        })
                    );
