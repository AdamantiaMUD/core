...
export const evt: StreamEventListenerFactory<CharacterNameCheckPayload> = {
    ...
    listener:
        (): StreamEventListener<CharacterNameCheckPayload> =>
        (
            stream: TransportStream<EventEmitter>,
            args: CharacterNameCheckPayload
        ): void => {
            stream.write(
                `{bold ${args.name} doesn't exist, would you like to create it?} {cyan [y/n]} `
            );

            stream.socket.once('data', (buf: Buffer) => {
                stream.write('');

                const confirmation = buf.toString().trim().toLowerCase();
                ...
                if (confirmation === 'n') {
                    ...
                    stream.dispatch(
                        new CreateCharacterEvent({ account: args.account })
                    );
