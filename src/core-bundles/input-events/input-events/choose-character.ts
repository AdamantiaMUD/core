...
export const evt: StreamEventListenerFactory<ChooseCharacterPayload> = {
    ...
    listener:
        (state: GameStateData): StreamEventListener<ChooseCharacterPayload> =>
        (
            ...
            const characters = account.characters.filter(
                (currChar: CharacterBrief) => !currChar.isDeleted
            );
                    ...
                    options.push({
                        ...
                        onSelect: async () => {
                            ...
                            if (hasValue(player)) {
                                ...
                                player.socket.dispatch(
                                    new CommandLoopEvent({ player })
                                );
            ...
            options.push({
                ...
                onSelect: () => {
                    ...
                    stream.socket.once('data', (buf: Buffer) => {
                        ...
                        stream.write(
                            `Deleting account {bold ${account.username}}`
                        );

                        account.deleteAccount();

                        stream.write(
                            'Account deleted, it was a pleasure doing business with you.'
                        );
            ...
            options.forEach(
                (opt: { display: string; onSelect?: () => void }) => {
