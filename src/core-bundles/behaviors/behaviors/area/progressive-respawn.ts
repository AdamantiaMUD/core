...
export const progressiveRespawn: BehaviorDefinition = {
    listeners: {
        [UpdateTickEvent.getName()]: (
            state: GameStateData
        ): MudEventListener<[Area, UpdateTickPayload]> => {
            ...
            return (area: Area, payload: UpdateTickPayload): void => {
                ...
                const respawnInterval: number =
                    cast<number>(config.interval) > 0
        ...
        [AreaRoomAddedEvent.getName()]:
            (): MudEventListener<[Area, AreaRoomAddedPayload]> =>
            (area: Area, { room }: AreaRoomAddedPayload): void => {
                room.listen(
                    ...
                    (rm: Room, { state }: RoomRespawnTickPayload): void => {
                        ...
                        rm.defaultNpcs.forEach(
                            (entity: RoomEntityDefinition) => {
                                ...
                                const entityCount = [...rm.spawnedNpcs].filter(
                                    (npc: Npc) =>
                                        npc.entityReference ===
                                        entityDefinition.id
                                ).length;
                                ...
                                if (
                                    probability(
                                        entityDefinition.respawnChance ?? 0
                                    )
                                ) {
                                    try {
                        ...
                        rm.defaultItems.forEach(
                            (entity: RoomEntityDefinition) => {
                                ...
                                const entityCount = [...rm.items].filter(
                                    (item: Item) =>
                                        item.entityReference === entityDef.id
                                ).length;

                                const shouldRespawn =
                                    entityCount < (entityDef.maxLoad ?? 1);

                                if (
                                    (!shouldRespawn &&
                                        !entityDef.replaceOnRespawn) ||
                                    !hasValue(state)
                                ) {
                                    return;
                                }
