...
export const lootable: BehaviorDefinition = {
    listeners: {
        [NpcKilledEvent.getName()]:
            (
                state: GameStateData
            ): MudEventListener<[Npc, NpcKilledPayload, SimpleMap]> =>
            async (
                npc: Npc,
                ...
                const items: Item[] = roll
                    .map((item: string): Item | null =>
                        state.itemFactory.create(item, area)
                    )
                    .filter(hasValue);
                ...
                if (hasValue(killer) && killer instanceof Player) {
                    if (hasValue(currencies)) {
                        currencies.forEach(
                            (currency: { amount: number; name: string }) => {
                                // distribute currency among group members in the same room
                                const recipients = (
                                    hasValue(killer.party)
                                        ? [...killer.party]
                                        : [killer]
                                ).filter(
                                    (recipient: Player) =>
                                        recipient.room === killer.room
                                );
